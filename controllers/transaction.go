...
func (ae AppEngine) CreateExpenseTrx(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		ae.Storage.DB.Raw("SELECT id FROM categories WHERE name = ? and user_id = ?", categoryName, user_id).Scan(&categoryID)
		...
		ae.Storage.DB.Where("id = ?", category_id).First(&transactionCategory)
